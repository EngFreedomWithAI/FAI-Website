...
var html = /* @__PURE__ */ __name((markup, status = 200) => new Response(markup, {
  status,
...
var statusPage = /* @__PURE__ */ __name((siteUrl, title, body) => `<!doctype html>
<html lang="en"><head>
<meta charset="utf-8" />
...
// _lib/ses.ts
var asArray = /* @__PURE__ */ __name((v) => Array.isArray(v) ? v : [v], "asArray");
var clean = /* @__PURE__ */ __name((v) => (v ?? "").replace(/\s+/g, ""), "clean");
...
var handleAdvisoryPost = /* @__PURE__ */ __name(async ({ request, env }) => {
...
var handleContactPost = /* @__PURE__ */ __name(async ({ request, env }) => {
  const data = await readBody(request);
  if (data.company) return json({ ok: true });
  const name = data.name ?? "";
  const email = (data.email ?? "").toLowerCase();
...
var MAX_GOAL = 2e3;
var prettyDate = /* @__PURE__ */ __name((iso) => {
...
var handleOfficeHoursPost = /* @__PURE__ */ __name(async ({ request, env }) => {
  const data = await readBody(request);
  if (data.company) return json({ ok: true });
...
// ../.wrangler/tmp/pages-AT6SGx/functionsRoutes-0.3247998454721146.mjs
var routes = [
  {
...
aws4fetch/dist/aws4fetch.esm.mjs:
   ...
   *)
*/
//# sourceMappingURL=functionsWorker-0.5422269686692696.mjs.map
